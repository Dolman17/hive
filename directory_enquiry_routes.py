...
def log_enquiry_event(enquiry, event_type, notes=None, created_by=None, created_by_label=None, commit=False):
    event = DirectoryEnquiryEvent(
        ...
        created_by_label=created_by_label or (
            created_by.name if created_by and created_by.is_authenticated else "Public enquiry form"
        ),
    )
...
def enquiry_admin_url(enquiry):
    return url_for(
        "directory_enquiries.admin_directory_enquiry_detail",
        enquiry_id=enquiry.id,
    )


def enquiry_consultant_url(enquiry):
    return url_for(
...
def public_directory_enquiry(profile_id):
    ...
    if request.method == "POST":
        ...
        if notify_admins:
            notify_admins(
                title="New directory enquiry submitted",
                message=f"{enquiry.company_name} submitted a directory enquiry.",
                category="directory_enquiry",
                link_url=enquiry_admin_url(enquiry),
            )
        ...
        if enquiry.assigned_consultant_id:
            ...
            if create_notification:
                create_notification(
                    recipient_user_id=enquiry.assigned_consultant_id,
                    title="New directory enquiry linked to you",
                    message=f"{enquiry.company_name} submitted an enquiry through your public profile.",
                    category="directory_enquiry",
                    link_url=enquiry_consultant_url(enquiry),
                )
        ...
        return redirect(
            url_for(
                "directory_enquiries.public_directory_enquiry_thanks",
                enquiry_id=enquiry.id,
            )
        )
...
def admin_directory_enquiries():
    ...
    return render_template(
        "directory_enquiries/admin_list.html",
        enquiries=enquiries,
...
def admin_directory_enquiry_detail(enquiry_id):
    ...
    if request.method == "POST":
        ...
        if previous_status != enquiry.status:
            log_enquiry_event(
                enquiry,
                "Status Changed",
                f"Status changed from {previous_status} to {enquiry.status}.",
        ...
        if previous_consultant_id != enquiry.assigned_consultant_id:
            if enquiry.assigned_consultant_id:
                ...
                log_enquiry_event(
                    enquiry,
                    "Consultant Assigned",
                    f"Enquiry assigned to {assigned_name}.",
                    created_by=current_user,
                )
                ...
                if create_notification:
                    create_notification(
                        recipient_user_id=enquiry.assigned_consultant_id,
                        title="Directory enquiry assigned to you",
                        message=f"{enquiry.company_name} has been assigned to you.",
                        category="directory_enquiry",
                        link_url=enquiry_consultant_url(enquiry),
                    )
            else:
                log_enquiry_event(
                    ...
                    "The enquiry was moved back to unassigned.",
                    created_by=current_user,
                )
    ...
    return render_template(
        "directory_enquiries/admin_detail.html",
        enquiry=enquiry,
        consultants=consultants,
    )
...
def consultant_directory_enquiry_detail(enquiry_id):
    enquiry = (
        DirectoryEnquiry.query
        .filter_by(id=enquiry_id, assigned_consultant_id=current_user.id)
        .first_or_404()
    )
    ...
    if request.method == "POST":
        ...
        if previous_status != enquiry.status:
            ...
            if notify_admins:
                notify_admins(
                    title="Directory enquiry status changed",
                    message=f"{current_user.name} changed {enquiry.company_name} from {previous_status} to {enquiry.status}.",
                    category="directory_enquiry",
                    link_url=enquiry_admin_url(enquiry),
                )
        ...
        if enquiry.consultant_notes:
            log_enquiry_event(
                enquiry,
                "Consultant Notes Updated",
                "Consultant notes were updated.",
                created_by=current_user,
            )
...
def admin_directory_enquiry_timeline(enquiry_id):
    ...
    return render_template(
        "directory_enquiries/timeline.html",
        enquiry=enquiry,
        events=events,
        viewer="admin",
    )
